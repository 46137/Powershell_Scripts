function BT-Netstat(){
    ...
    To Run: Import-Module .\BT-Netstat.ps1 -Force 
     ...
     UDP equivilent.
    executablepath or just path from get-process
    ...
    #Raw data.
    $raw = Get-NetTCPConnection
    
    # Creating an array to store filtered information.
    $Filtered = @()
    $raw |ForEach-Object {
        $Return_Data = New-Object -TypeName PSObject |Select-Object -Property CreationTime, PID, ProcessName, State, SrcAddress, SrcPort, DstAddress, DstPort, NetworkProfile, ExecutablePath 
        ...
        #$Return_Data.ExecutablePath = (Get-Process -Id $_.OwningProcess).ExecutablePath
