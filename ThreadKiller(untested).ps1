function Get-ProcInfo
{
  ...
  Lists processes, modules, and threads using CreateToolhelp32Snapshot.
  ...
  Author - Daniel Sweet (daniel.sweet@tanium.com)
  ...
  $hSnapshot = CreateToolhelp32Snapshot -ProcessId 0 -Flags $TH32CS_SNAPALL

  $hSnapshot | Get-Member

  if($Processes)
  {
    $Process = Process32First -SnapshotHandle $hSnapshot
    ...
    do
    {
      ...
      if($Process.th32ProcessId -ne 0 -and $Process.th32ProcessId -ne 4)
      {
        $hProcess = OpenProcess -ProcessId $Process.th32ProcessId -DesiredAccess $PROCESS_ALL_ACCESS -InheritHandle $false

        if($hProcess -ne 0)
        {
          $KernelPath = QueryFullProcessImageName -ProcessHandle $hProcess

          $ProcessDetail = New-Object PSObject
          $ProcessDetail | Add-Member -MemberType Noteproperty -Name ProcessId -Value $Process.th32ProcessID
          $ProcessDetail | Add-Member -MemberType Noteproperty -Name ParentProcessId -Value $Process.th32ParentProcessID
          $ProcessDetail | Add-Member -MemberType Noteproperty -Name KernelPath -Value $KernelPath
          $ProcessDetail | Add-Member -MemberType Noteproperty -Name ProcessSize -Value $Process.dwSize
          Write-Output $ProcessDetail
        }
        CloseHandle($hProcess)
      }
    } while($Kernel32::Process32Next($hSnapshot, [ref]$Process))
  }

  if($Modules)
  {
    $Module = Module32First -SnapshotHandle $hSnapshot

    $Module | Get-Member
    do
    {
        if($Module.th32ProcessID -ne 0 -and $Module.th32ProcessID -ne 4)
        {
          $ModuleDetail = New-Object PSObject
          $ModuleDetail | Add-Member -MemberType Noteproperty -Name ProcessId -Value $Module.th32ProcessID
          $ModuleDetail | Add-Member -MemberType Noteproperty -Name Module -Value $Module.szModule
          $ModuleDetail | Add-Member -MemberType Noteproperty -Name ExePath -Value $Module.szExePath
          $ModuleDetail | Add-Member -MemberType Noteproperty -Name ModuleSize -Value $Module.modBaseSize
          Write-Output $ModuleDetail
        }
    } while($Kernel32::Module32Next($hSnapshot, [ref]$Module))
  }
      ...
      $Thread
        ...
        if($Thread.th32OwnerProcessId -ne 0 -and $Thread.th32OwnerProcessId -ne 4)
        {
          ...
          if($hThread -ne 0)
          {
            ...
            $ThreadDetail | Add-Member -MemberType Noteproperty -Name OwnerProcessId -Value $Thread.th32OwnerProcessId
            $ThreadDetail | Add-Member -MemberType Noteproperty -Name ThreadSize -Value $Thread.dwSize
...
function Get-InjectedThread
{
    ...
    $hSnapshot = CreateToolhelp32Snapshot -ProcessId 0 -Flags $TH32CS_SNAPTHREAD
...
#region PSReflect Definitions (Win32)

$Mod = New-InMemoryModule -ModuleName Win32

$LuidAttributes = psenum $Mod Win32.LuidAttributes UInt32 @{
    DISABLED                            =   '0x00000000'
...
$MemProtection = psenum $Mod Win32.MemProtection UInt32 @{
    PAGE_EXECUTE            = 0x10
...
$MemState = psenum $Mod Win32.MemState UInt32 @{
    MEM_COMMIT  = 0x1000
    MEM_RESERVE = 0x2000
    MEM_FREE    = 0x10000
}

$MemType = psenum $Mod Win32.MemType UInt32 @{
    MEM_PRIVATE = 0x20000
    MEM_MAPPED  = 0x40000
    MEM_IMAGE   = 0x1000000
}

$SecurityEntity = psenum $Mod Win32.SecurityEntity UInt32 @{
...
$SidNameUser = psenum $Mod Win32.SID_NAME_USE UInt32 @{
  SidTypeUser                            = 1
  SidTypeGroup                           = 2
  SidTypeDomain                          = 3
  SidTypeAlias                           = 4
...
$TokenInformationClass = psenum $Mod Win32.TOKEN_INFORMATION_CLASS UInt16 @{
  TokenUser                             = 1
  TokenGroups                           = 2
  TokenPrivileges                       = 3
  TokenOwner                            = 4
...
#$MAX_PATH = psenum $Mod Win32.MAX_PATH Char @{}

$LUID = struct $Mod Win32.Luid @{
    LowPart         = field 0 $SecurityEntity
    HighPart        = field 1 Int32
}

$LUID_AND_ATTRIBUTES = struct $Mod Win32.LuidAndAttributes @{
    Luid            = field 0 $LUID
    Attributes      = field 1 UInt32
}

$MEMORYBASICINFORMATION = struct $Mod Win32.MEMORY_BASIC_INFORMATION @{
...
$SID_AND_ATTRIBUTES = struct $Mod Win32.SidAndAttributes @{
    Sid             = field 0 IntPtr
    Attributes      = field 1 UInt32
}

$MODULEENTRY32 = struct $Mod Win32.MODULEENTRY32 @{
    dwSize          = field 0 UInt32
    th32ModuleID    = field 1 UInt32
    th32ProcessID   = field 2 UInt32
    GlblcntUsage    = field 3 UInt32
    ProccntUsage    = field 4 UInt32
    modBaseAddr     = field 5 Byte
    modBaseSize     = field 6 UInt32
    hModule         = field 7 IntPtr
    szModule        = field 8 UInt16[] -MarshalAs @('ByValArray', 256)
    szExePath       = field 9 UInt16[] -MarshalAs @('ByValArray', 260)
}

$PROCESSENTRY32 = struct $Mod Win32.PROCESSENTRY32 @{
    dwSize              = field 0 UInt32
    cntUsage            = field 1 UInt32
    th32ProcessID       = field 2 UInt32
    th32DefaultHeapID   = field 3 UIntPtr
    th32ModuleID        = field 4 UInt32
    cntThreads          = field 5 UInt32
    th32ParentProcessID = field 6 UInt32
    pcPriClassBase      = field 7 UInt32
    dwFlags             = field 8 UInt32
    szExeFile           = field 9 String -MarshalAs @('ByValTStr', 260)
}

$THREADENTRY32 = struct $Mod Win32.THREADENTRY32 @{
    dwSize              = field 0 UInt32
    cntUsage            = field 1 UInt32
...
$TOKEN_MANDATORY_LABEL = struct $Mod Win32.TokenMandatoryLabel @{
    Label           = field 0 $SID_AND_ATTRIBUTES;
}

$TOKEN_ORIGIN = struct $Mod Win32.TokenOrigin @{
  OriginatingLogonSession = field 0 UInt64
}

$TOKEN_PRIVILEGES = struct $Mod Win32.TokenPrivileges @{
    PrivilegeCount  = field 0 UInt32
    Privileges      = field 1 $LUID_AND_ATTRIBUTES.MakeArrayType() -MarshalAs @('ByValArray', 50)
}

$TOKEN_USER = struct $Mod Win32.TOKEN_USER @{
...
$FunctionDefinitions = @(
    ...
    (func kernel32 Module32First ([bool]) @(
        [IntPtr],                                  #_In_    HANDLE          hSnapshot,
        $MODULEENTRY32.MakeByRefType()            #_Inout_ LPMODULEENTRY32 lpte
    ) -SetLastError)

    (func kernel32 Module32Next ([bool]) @(
        [IntPtr],                                  #_In_  HANDLE          hSnapshot,
        $MODULEENTRY32.MakeByRefType()            #_Out_ LPMODULEENTRY32 lpte
    ) -SetLastError),

    (func kernel32 Process32First ([bool]) @(
        [IntPtr],                                  #_In_    HANDLE          hSnapshot,
        $PROCESSENTRY32.MakeByRefType()            #_Inout_ LPPROCESSENTRY32 lpte
    ) -SetLastError)

    (func kernel32 Process32Next ([bool]) @(
        [IntPtr],                                  #_In_  HANDLE          hSnapshot,
        $PROCESSENTRY32.MakeByRefType()            #_Out_ LPPROCESSENTRY32 lpte
    ) -SetLastError),
...
$TH32CS_SNAPALL = $TH32CS_SNAPHEAPLIST -bor
                  $TH32CS_SNAPMODULE -bor
                  $TH32CS_SNAPMODULE32 -bor
                  $TH32CS_SNAPPROCESS -bor
                  $TH32CS_SNAPTHREAD

#endregion PSReflect Definitions (Win32)
...
function Module32First
{
    ...
    $Module = [Activator]::CreateInstance($MODULEENTRY32)
    $Module.dwSize = $MODULEENTRY32::GetSize()

    $Success = $Kernel32::Module32First($hSnapshot, [Ref]$Module); $LastError = [Runtime.InteropServices.Marshal]::GetLastWin32Error()

    if(-not $Success)
    {
        Write-Debug "Module32First Error: $(([ComponentModel.Win32Exception] $LastError).Message)"
    }
...
function Process32First
{
    ...
    $Process = [Activator]::CreateInstance($PROCESSENTRY32)
    $Process.dwSize = $PROCESSENTRY32::GetSize()

    $Success = $Kernel32::Process32First($hSnapshot, [Ref]$Process); $LastError = [Runtime.InteropServices.Marshal]::GetLastWin32Error()

    if(-not $Success)
    {
        Write-Debug "Process32First Error: $(([ComponentModel.Win32Exception] $LastError).Message)"
    }
